...
==== `es.scroll.size` now defaults to `1000`

In 8.0, we increased the default `es.scroll.size` to `1000` from `50`. If you
deploy ES-hadoop in a low memory environment, consider lowering `es.scroll.size`
to avoid issues.

==== Support for Scala 2.10 has been removed

In 8.0, our compatibility artifacts no longer support Scala 2.10. Spark deprecated
