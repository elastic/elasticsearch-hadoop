...
Note: Deserialization Error Handlers only allow handling of errors that occur when reading JSON data. It may be possible
that a search result is successfully read, but is still malformed, thus causing an exception when it is used in a
completely different part of the framework. This Error Handler is called from the top of the most reasonable place to
handle exceptions in the scroll reading process, but this does not encapsulate all possible reading logic for all
integrations.
