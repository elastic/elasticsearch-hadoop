...
val options13 = Map("path" -> "spark/index", 
                    "pushdown"<1> -> "true", 
                    "nodes"<2> -> "someNode", "esPort"<3> -> "9200")

// Spark 1.3 style
val spark13DF = sql.load("org.elasticsearch.spark.sql", options13<4>)
...
val spark14DF = sql.read().format("org.elasticsearch.spark.sql")
                          .options<4>(options).load("spark/index")
----
...
<4> Since using +.+ (or +_+ in Spark 1.4) can cause syntax exceptions, one should replace it instead with camelCase style. Thus, in this example +es.scroll.size+ becomes +scrollSize+ (as the leading +es+ can be removed)
