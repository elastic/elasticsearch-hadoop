...
This significantly increases the performance of the queries and minimizes the CPU, memory and I/O on both Spark and {es} clusters as only the needed data is returned (as oppose to returning the data bulk only to be processed and discarded by Spark).
