...
+DataFrame+ vs +SchemaRDD+:: The core unit of SparkSQL in 1.3 is a +DataFrame+ while previously it was a +SchemaRDD+.
Unified API vs dedicated Java/Scala APIs:: In SparkSQL 1.3 there is only one API for both Java and Scala, previous versions had dedicated APIs in particular with regards to data types.
...
val people = sqlContext.esDF("spark/people") <3>
...
For Java users, a dedicated API exists through +JavaEsSparkSQL+. It is stickingly similar to +EsSparkSQL+ however it allows configuration options to be passed in through Java collections instead of Scala ones; other than that using the two is exactly the same:
