...
Spark SQL is a young component, going through significant changes between releases. Spark SQL became a stable component in version 1.3, however it is https://spark.apache.org/docs/latest/sql-programming-guide.html#migration-guide[*not* backwards compatible] with the previous releases. {eh} supports both version Spark SQL 1.1-1.2 and 1.3 through two different jars:
+elasticsearch-spark-<version>.jar+ and +elasticsearch-hadoop-<version>.jar+ support Spark SQL 1.3 while +elasticsearch-spark-1.2-<version>.jar+ supports Spark SQL 1.1 and 1.2. In other words, if you are not using Spark SQL 1.3, append the +-1.2+ suffix to the {eh} artifact id.
...
Once created, the +DataFrame+ can be accessed freely to manipulate the data.
...
<4> Since using the +.+ can cause syntax exceptions, one should replace it instead with +_+ (underline). Thus, in this example +es.scroll.size+ becomes +scroll_size+ (as the leading +es+ can be removed)
...
For Java users, a dedicated API exists through +JavaEsSpark SQL+. It is stickingly similar to +EsSpark SQL+ however it allows configuration options to be passed in through Java collections instead of Scala ones; other than that using the two is exactly the same:
