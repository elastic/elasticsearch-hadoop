...
[[spark-sql-streaming]]
[float]
=== Spark Structured Streaming support [Alpha]
...
Released as an alpha feature in Spark 2.0, Spark Structured Streaming provides a unified streaming and batch interface
built into the Spark SQL integration. As of {eh} 6.0, we provide native functionality to index streaming data into {es}.

NOTE: Spark Structured Streaming (available in Spark SQL 2.0+) is still in alpha as of 2.1.1. As such, {eh} support for
Structured Streaming (available in {eh} 6.0+) is considered an alpha feature. It is not advised to use this for
production code.
...
Spark Structured Streaming was introduced in Spark 2.0 and is still in alpha as of Spark 2.1.1. Similar to Spark SQL
before it, Structured Streaming may be subject to significant changes between releases. While {eh} supports Spark
versions 1.3-1.6 as compatibility releases, Spark Structured Streaming is only supported in the
+elasticsearch-spark-20-<version>.jar+ artifact.
