...
Do note that one can also specify +es.+ options: simply declare them as such (but do use +_+ instead of +.+ as otherwise Spark will complain about the syntax); additionally one can simply skip the +es.+ prefix which the connector will add automatically.
...
----
val sql = new SQLContext...
val options = Map("pushdown"<1> -> "true", "nodes"<2> -> "es_node", "es_port"<3> -> "9200")
...
<3> +es.port+ specified in full format - notice the +.+ is replaced with +_+
...
Further more, the pushdown filters 
