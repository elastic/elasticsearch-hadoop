...
names on the {krb} principals match the user names LDAP or Active Directory exactly.
...
The delegation tokens from {es} that are retrieved are capped at a maximum lifetime of 7 days. After that time passes,
the tokens will no longer successfully authenticate connections. Since streaming jobs are expected to run continuously
without stopping, you should configure Spark so that the credential provider can obtain new tokens before the original
tokens expire.
