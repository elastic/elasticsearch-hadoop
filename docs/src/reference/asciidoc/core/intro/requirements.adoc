...
Spark 1.0.0 or higher. We recommend using the latest release of Spark (currently 1.3.0). As {eh} provides
...
----
$ pwd
/libs/spark/spark-1.3.0-bin-XXXXX
----
...
include +spark-sql-<scala-version>.jar+ or the Spark _assembly_ : +spark-assembly-1.3.0-<distro>.jar+

{eh} supports Spark SQL 1.3.0 through the main jar. Since Spark SQL 1.3 is _not_ 
