...
Spark 1.0.0 or higher. We recommend using the latest release of Spark (currently 1.1.1). As {eh} provides
...
----
$ pwd
/libs/spark/spark-1.1.1-bin-XXXXX
----
...
include +spark-sql-<scala-version>.jar+ or the Spark _assembly_ : +spark-assembly-1.1.0-<distro>.jar+

Spark SQL 1.1.0 or higher (and thus Spark 1.1.0) is required. Note that Spark SQL is currently an _alpha_ component

[[requirements-storm]]
=== Apache Storm

Spark 0.9.2 or higher. We recommend using the latest release of Storm (currently 0.9.3).
