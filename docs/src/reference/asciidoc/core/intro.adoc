...
[partintro]
--
{ehtm} is an <<license, open-source>>, stand-alone, self-contained, small library that allows Hadoop jobs (whether using {mr} or libraries built upon it such as Hive, or Pig or new upcoming libraries like {sp} ) to 'interact' with {es}. One can think of it as a _connector_ that allows data to flow 'bi-directionaly' so that applications can leverage transparently the {es} engine capabilities to significantly enrich their capabilities and increase the performance.

{ehtm} offers first-class support for vanilla {mr}, Pig and Hive so that using {es} is literally like using resources within the Hadoop cluster. As such,
