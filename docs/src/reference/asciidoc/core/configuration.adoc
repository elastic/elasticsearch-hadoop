...
To put everything together, if you want to enable `DEBUG` logging on the {mr} package make sure the `log4j.properties` (used by your environment) has the following content (under Hadoop 2.x):
