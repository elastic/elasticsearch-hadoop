...
ignored if you are reading from an {es} cluster that does not support scroll slicing ({es} any version below v5.0.0).

added[6.2.3]
`es.input.use.sliced.partitions` (default true)::
By default, when this parameter is true, sliced partitions are used (standard behavior for {es} v5.0.0 and above). This
...
use shard partitions (as for {es} before 5.0.0). deprecated[6.5.0] This setting is removed in 7.0.0, as the default
value for `es.input.max.docs.per.partition` has been removed, and sliced scrolls will be disabled by default unless a
