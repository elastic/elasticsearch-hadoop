...
1. *spark* artifact. Notice the `-30` part of the suffix which indicates the Spark version compatible with the artifact. Use `41` for Spark 4.1 and 4.2, `40` for Spark 4.0+, `35` for Spark 3.5+, and `30` for Spark 3.0+. Notice the `_2.13` suffix which indicates the Scala version compatible with the artifact. Currently it is the same as the version used by Spark itself.
